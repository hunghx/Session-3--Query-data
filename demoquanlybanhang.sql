...
select SUM(o.createdDate) from (select createdDate from `order` where id=1 ) o;

select sum(username) from user;

select round(avg(total))from `order`; -- 2871428.5714285714/ 2871429

select count(id) from `order` where user_id=3;

select order_id,productPrice,productName,quantity,MAX(productPrice* quantity) from `order_detail`;

select lcase(fullname), username,password from user; 
select length(fullname), username,password from user; 
select now();
select * from order_detail;
-- tính giá trị đơn hàng trung bình của từng người mua 
select  sum(total),count(id), user_id from `order` group by (user_id); 
-- thống kê số lượng trung bình mà khách hàng mua trong tháng theo từng sản phẩm và người mua;
select AVG(quantity) , o.user_id, od.productName
from order_detail od join `order` o on od.order_id = o.id group by od.product_id,o.user_id;
-- thống kê doanh thu cửa hàng trong tháng 4;
select sum(od.quantity* od.productPrice) 
from `order_detail`od join `order` o on o.id = od.order_id 
where month(o.createdDate) = 4; 
